...
function App(){
  ...
  const BonTransportView = () => {
    ...
    // Entrepôt sélectionné (objet complet)
    ...
    useEffect(()=>{
      ...
      // Pré-sélectionner le premier entrepôt
      const ents = settings.entrepots||[];
      if(ents.length>0) setForm(f=>({...f, entrepotId:ents[0].id}));
      // Pré-remplir livraison depuis base active
      if(state.activeClient && state.clients[state.activeClient]){
        const c = state.clients[state.activeClient];
        setForm(f=>({...f, livNom:c.name}));
      }
    },[]);
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;font-family:Arial,sans-serif;font-size:12px;color:#0D1B2A}
  ...
  body{padding:24px 30px;background:white;display:flex;flex-direction:column;min-height:100vh}
  .page{flex:1;display:flex;flex-direction:column}
  .main{flex:1}
  .header{display:grid;grid-template-columns:220px 1fr 180px;align-items:center;margin-bottom:16px;padding-bottom:12px;border-bottom:3px solid #2d7a2d}
  .bt-title{text-align:center;font-size:18px;font-weight:800;color:#0D1B2A;text-transform:uppercase;letter-spacing:2px}
  ...
  .box-green{background:#F0FDF4;border:1px solid #86EFAC;border-radius:6px;padding:12px}
  ...
  .sign-row{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:12px;padding-top:12px;border-top:1px solid #E8ECF1}
  .sign-zone{border:1px dashed #CBD5E0;border-radius:6px;height:56px}
  .footer{padding-top:10px;border-top:2px solid #2d7a2d;text-align:center;font-size:9px;color:#666;line-height:1.9;margin-top:auto}
  @media print{body{padding:10px 16px;-webkit-print-color-adjust:exact;print-color-adjust:exact}@page{margin:.7cm;size:A4}}
</style></head><body>
<div class="page">
<div class="main">
<div class="header">
  <div><img src="${LOGO_MRDPS27}" style="height:90px;width:auto;object-fit:contain"/></div>
...
<div class="grid2">
  <div class="box">
    ...
    ${selectedEntrepot ? `
    <div style="color:#2d7a2d;font-weight:700;margin-top:4px;font-size:12px">${selectedEntrepot.nom}</div>
    ...
    ${selectedEntrepot.tel ? '<div style="color:#4A5568;margin-top:3px">Tél : '+selectedEntrepot.tel+'</div>' : ''}
  ...
  <div class="box-green">
    ...
    <div style="font-weight:700;font-size:13px">${form.livNom||"—"}</div>
      ...
      <div className="anim">
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Card>
              <div style={{fontWeight:600,fontSize:13,color:T.txt,marginBottom:12}}>Expéditeur — Site d'envoi</div>
              ...
              <label style={lbl}>Choisir l'entrepôt / Site d'envoi</label>
              <select style={{...inp,marginBottom:8}} value={form.entrepotId} onChange={e=>setForm({...form,entrepotId:e.target.value})}>
              ...
              {entrepots.length===0&&<div style={{fontSize:12,color:T.muted,marginTop:6}}>⚠ Aucun entrepôt configuré — <button onClick={()=>setView("settings")} style={{background:"none",border:"none",cursor:"pointer",color:T.brand,fontSize:12,fontFamily:"inherit",fontWeight:600}}>Configurer dans Paramètres → Entrepôts</button></div>}
            </Card>
